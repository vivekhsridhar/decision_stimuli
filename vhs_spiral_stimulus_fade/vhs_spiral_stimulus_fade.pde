...
void setup() {
  ...
  for (int i = 0; i < total; i++) {
    ...
    plist.add(new Particle(PVector.random2D().mult(random(width)/1.5)));
  }
}
...
void draw() {
  ...
  if (!standby) {
    ...
    float scale = 2.0;
    if (!trial) {
      rotate(radians(frameCount) * rotation_scale);
      ...
      for (int i = 0; i < n_circles; ++i) {
        stroke(grey - i*n_circles, grey - i*n_circles, grey - i*n_circles, 50);
        fill(grey - i*n_circles, grey - i*n_circles, grey - i*n_circles, 50);
        ellipse(0.0, 0.0, centre_diameter + scale*i*centre_diameter/n_circles, centre_diameter + scale*i*centre_diameter/n_circles);
      }
      ...
      ellipse(-width/2 + side_bar_width, 0, refuge_diameter, refuge_diameter);
      ellipse(width/2 - side_bar_width, 0, refuge_diameter, refuge_diameter);
      //ellipse(width/2, 0.0, refuge_diameter, refuge_diameter);
      //ellipse(width/2, height, refuge_diameter, refuge_diameter);
      
      for (int i = 0; i < n_circles; ++i) {
        stroke(grey - i*n_circles, grey - i*n_circles, grey - i*n_circles, 50*timer/start_timer);
        fill(grey - i*n_circles, grey - i*n_circles, grey - i*n_circles, 50*timer/start_timer);
        ellipse(0.0, 0.0, centre_diameter + scale*timer/start_timer*i*centre_diameter/n_circles, centre_diameter + scale*timer/start_timer*i*centre_diameter/n_circles);
      }
...
void squareDisplay() {
  fill(0);
  if (!standby && !trial) rotate(-radians(frameCount) * rotation_scale);
  
  rect(-width/2, -height/2, side_bar_width, height);
  rect(width/2 - side_bar_width, -height/2, side_bar_width, height);
}